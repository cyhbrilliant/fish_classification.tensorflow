...
conv2=tf.nn.conv2d(conv1_bn,Wconv2,strides=[1,2,2,1],padding='SAME')+Bconv2
conv2_bn=tf.nn.relu(batch_norm(conv2,False))



Wconv3=weight_variable([3,3,64,96])
Bconv3=bias_variable([96])
conv3=tf.nn.conv2d(conv2_bn,Wconv3,strides=[1,1,1,1],padding='SAME')+Bconv3
conv3_bn=tf.nn.max_pool(tf.nn.relu(batch_norm(conv3,False)),ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')


Wconv4=weight_variable([3,3,96,128])
Bconv4=bias_variable([128])
conv4=tf.nn.conv2d(conv3_bn,Wconv4,strides=[1,1,1,1],padding='SAME')+Bconv4
conv4_bn=tf.nn.max_pool(tf.nn.relu(batch_norm(conv4,False)),ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')


Wconv5=weight_variable([3,3,128,192])
Bconv5=bias_variable([192])
...
Wconv6=weight_variable([3,3,192,256])
Bconv6=bias_variable([256])
conv6=tf.nn.conv2d(conv5_bn,Wconv6,strides=[1,1,1,1],padding='SAME')+Bconv6
conv6_bn=tf.nn.max_pool(tf.nn.relu(batch_norm(conv6,False)),ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')


Wconv7=weight_variable([3,3,256,384])
Bconv7=bias_variable([384])
...
Wconv8=weight_variable([3,3,384,512])
Bconv8=bias_variable([512])
conv8=tf.nn.conv2d(conv7_bn,Wconv8,strides=[1,1,1,1],padding='SAME')+Bconv8
conv8_bn=tf.nn.max_pool(tf.nn.relu(batch_norm(conv8,False)),ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')
conv8_bn_line=tf.reshape(conv8_bn,shape=[-1,2*3*512])
#
...
#
Wdense18=weight_variable([2*3*512,1024])
Bdense18=bias_variable([1024])
dense18=tf.nn.relu(tf.matmul(conv8_bn_line,Wdense18)+Bdense18)
...
saver.restore(sess,'session_class2_small/session1.ckpt')

Test_out=[]
for i in range(40):
    print(i)
    Test_out.append(sess.run(OUT,feed_dict={Xp:DATA_ALL[i*25:(i+1)*25,:,:,:],Yp:np.zeros([25,2])}))

Test_out=np.array(Test_out)
Test_out=np.reshape(Test_out,[1000,2])
...
for i in range(1000):
    for j in range(2):
        if Test_out[i,j]>0.95:
            Test_out[i,j]=1
        if Test_out[i,j]<0.03:
            Test_out[i,j]=0
np.savetxt('class2_testout.csv', Test_out, fmt='%s', delimiter=',')
